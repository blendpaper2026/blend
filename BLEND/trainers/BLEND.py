...
def load_clip_to_cpu(cfg):
    ...
    model = clip.build_model_BLEND(state_dict or model.state_dict(), cfg.TRAINER.BLEND.PER_LORA_DIM)
    return model
...
class TextEncoder(nn.Module):
    def __init__(self, clip_model):
        ...
        proj_dim = self.text_projection.shape[1]
    ...
    def forward(self, prompts, tokenized_prompts, retrun_adapater_func=None):
        ...
        if retrun_adapater_func == None:
            ...
            x, _= self.transformer([x, retrun_adapater_func, x])
...
class AdapterLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.text_adapter = self._build_adapter(
            ...
            cfg.TRAINER.BLEND.ADAPTER_LAYERS_TXT,
            cfg.TRAINER.BLEND.ADAPTER_DIM,
            clip_model.dtype
        )
        ...
        self.visual_adapter = self._build_adapter(
            ...
            cfg.TRAINER.BLEND.ADAPTER_LAYERS,
            cfg.TRAINER.BLEND.ADAPTER_DIM,
            clip_model.dtype,
            visual = True
        )

        self.adapter_scale = float(cfg.TRAINER.BLEND.ADAPTER_SCALE)

    def return_text_adapter(self, index):
        return self.text_adapter[index], self.adapter_scale

    def return_visual_adapter(self, index):
        return self.visual_adapter[index], self.adapter_scale


    def _build_text_embedding(self, cfg, classnames, clip_model):
        dtype = clip_model.dtype
        text_ctx_init = cfg.TRAINER.BLEND.TEXT_CTX_INIT
    ...
    def _build_adapter(self, d_model, n_layers, adapter_layers, mid_dim, dtype, visual=False, zeros = False):
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.gamma_importance = cfg.TRAINER.BLEND.GAMMA_IMPORTANCE
    ...
    def encode_image(self, image, visual_adapter_func=None, visual_adapter_func_gen = None):
        if visual_adapter_func is not None:
            image_features = self.image_encoder(
                [image.type(self.dtype), visual_adapter_func, visual_adapter_func_gen]
            )
        else:
            image_features = self.image_encoder(
    ...
    def forward(self, image, idx=None):
        ...
        G_head_features, P_head_features = self.encode_image(image, visual_adapter_func)
        ...
        if not self.training:
            with torch.no_grad():
                self.all_features.append(G_head_features.cpu())
                self.all_features_text.append(text_features.cpu())

        logit_scale = self.logit_scale.exp()
        anchor_logits = logit_scale * (G_head_features) @ text_features.t()
        fusion_logits = logit_scale * ((1-self.gamma_importance)*G_head_features+self.gamma_importance*P_head_features) @ (text_features.t())


        return anchor_logits, fusion_logits


class BLEND(TrainerX):

    def check_cfg(self, cfg):
        assert cfg.TRAINER.BLEND.PREC in ["fp16", "fp32", "amp"]

    def build_model(self):
        cfg = self.cfg
        classnames = self.dm.dataset.classnames
        self.Lambda= cfg.TRAINER.BLEND.LAMBDA

        print(f"Loading CLIP (backbone: {cfg.MODEL.BACKBONE.NAME})")
        clip_model = load_clip_to_cpu(cfg)
        
        if cfg.TRAINER.BLEND.PREC == "fp32" or cfg.TRAINER.BLEND.PREC == "amp":
        ...
        names_to_update = ["text_adapter", "visual_adapter", "shared_adapter", "proj_adapter"]
        ...
        self.scaler = GradScaler() if cfg.TRAINER.BLEND.PREC == "amp" else None
    ...
    def forward_backward(self, batch_idx, batch, idx=-1, **kwargs):
        ...
        prec = self.cfg.TRAINER.BLEND.PREC
        if prec == "amp":
            with autocast():
                ...
                loss =(1-self.Lambda)*running_anchor_loss + self.Lambda*representation_fusion_loss
