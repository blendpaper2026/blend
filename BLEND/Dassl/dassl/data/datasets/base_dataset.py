...
class DatasetBase:
    ...
    def __init__(self, cfg, train_x=None, federated_train_x=None, train_u=None, val=None, federated_test_x=None, test=None):
        self._federated_train_x_new, self._federated_test_x_new = self.generate_federated_fewshot_dataset_new(train_x, test, num_shots = cfg.DATASET.NUM_SHOTS, num_users = cfg.DATASET.USERS, alpha= cfg.DATASET.DIR_ALPHA, boost = 1)
    ...
    def val(self):
        return self._val
    @property
    def federated_train_x_new(self):
        return self._federated_train_x_new

    @property
    def federated_test_x_new(self):
        return self._federated_test_x_new
    ...
    def generate_federated_fewshot_dataset_new(self, data_source, test, num_shots=16, num_users=10,alpha=0.01,boost=1.0):
        print(f"Dirichlet (base alpha={alpha}, boost={boost}), num_shots={num_shots}")
        rng = np.random.default_rng(1)
        random.seed(1)

        output_dict = defaultdict(list)

        tracker = self.split_dataset_by_label(data_source)  # {label: [items]}
        tracker_test = self.split_dataset_by_label(test)
        
        class_num = len(tracker)
        print("num_classes:", class_num)

        for class_id, (label, items) in enumerate(tracker.items()):
            # how many from this class
            total_needed = num_shots
            if total_needed == 0:
                continue

            # 1) build an asymmetric Dirichlet for THIS class
            # base alpha for everyone
            alpha_vec = np.full(num_users, alpha, dtype=float)
            # pick a "preferred" user for this class, rotate over users
            preferred_user = class_id % num_users
            alpha_vec[preferred_user] += boost

            # 2) sample proportions + counts
            probs = rng.dirichlet(alpha_vec)
            counts = rng.multinomial(total_needed, probs)

            # 3) assign items
            pool = items if total_needed >= len(items) else random.sample(items, total_needed)
            random.shuffle(pool)
            start = 0
            users = [u for u in range(num_users)]
            users = list(rng.permutation(users))
            for u in users:
                c = int(counts[u])
                if c > 0:
                    end = start + c
                    output_dict[u].extend(pool[start:end])
                    start = end

        # users with no samples
        empties = [u for u in range(num_users) if len(output_dict[u]) == 0]
        if empties:
            # max-heap by "spare" = (len-1); only users with >1 can spare at least one
            # store as (-spare, user) to simulate max-heap with heapq
            heap = [ (-(len(items) - 1), u) for u, items in output_dict.items() if len(items) > 1 ]
            heapify(heap)

            for e in empties:
                if not heap:
                    # fallback: all users have ≤1; take from the current largest anyway
                    donor = max(range(num_users), key=lambda u: len(output_dict[u]))
                else:
                    _, donor = heappop(heap)

                # move one random item from donor to empty user (preserves class totals overall)
                idx = random.randrange(len(output_dict[donor]))
                item = output_dict[donor].pop(idx)
                output_dict[e].append(item)

                # if donor still has >1 left, it can spare another later; push back with updated spare
                spare = len(output_dict[donor]) - 1
                if spare > 0:
                    heappush(heap, (-spare, donor))

        # debug
        for u in range(num_users):
            print(f"user {u}: {len(output_dict[u])} samples")


        test_dict = defaultdict(list)
        for u in range(num_users):
            tracker_u = self.split_dataset_by_label(output_dict[u])
            for label in tracker_u.keys():
                test_dict[u].extend(tracker_test[label])

        for u in range(num_users):
            tracker_u = self.split_dataset_by_label(test_dict[u])
            print(f"Data points of user {u}: {tracker_u.keys()}")

        # optional plot
        # self.plot_label_portions_per_user(cfg, output_dict)

        return output_dict, test_dict
